...
+ 有 parent 上下文时冻结 parent system prompt 与 Observational Memory summary
+ 真正空会话只声明显式 empty-parent root，不携带 parent system prompt
...
- `created`：结构化持久一次 summary 或 empty-parent 描述、coverage/概念根、模型、模板、side identity，以及普通 OM epoch 的 parent system；empty-parent epoch 不保存其内容。恢复时重建 seed message 及可派生 hash，不再双重 JSON 编码大字段；
...
- side 模型变化；
- side template 变化；
- parent system prompt 变化（普通 OM epoch）；
...
- empty-parent bootstrap 只支持真正没有 parent source entries 且没有 OM ledger 的首次启动；它不把父系统提示词作为 side 上下文；“已有历史但 OM 尚未覆盖”仍不支持并显式失败；
